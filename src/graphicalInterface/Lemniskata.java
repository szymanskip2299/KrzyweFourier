...
public class Lemniskata {
	...
	public Lemniskata(CenterPanel cent,int el) {
		elements=el;
		centerPanel=cent;
		int t;
    	for(t=0;t<x.length;t+=1) {
			double xx=(200*Math.sqrt(2)*Math.cos(2*Math.PI*t/1000))/(Math.pow(Math.sin(2*Math.PI*t/1000),2)+1);
			double yy=(200*Math.sqrt(2)*Math.cos(2*Math.PI*t/1000)*Math.sin(2*Math.PI*t/1000))/(Math.pow(Math.sin(2*Math.PI*t/1000),2)+1);
			x[t]=(int)xx;
			y[t]=(int)yy;
		 }
