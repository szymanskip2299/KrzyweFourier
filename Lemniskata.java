...
public class Lemniskata extends JPanel {
	...
	public Lemniskata(CenterPanel cent,int el) {
		...
		centerPanel=cent;
		int t;
    	for(t=0;t<=x.length-1;t+=2) {
			double xx=(200*Math.sqrt(2)*Math.cos(t))/(Math.pow(Math.sin(t),2)+1);
			double yy=(200*Math.sqrt(2)*Math.cos(t)*Math.sin(t))/(Math.pow(Math.sin(t),2)+1);
			x[t]=(int)xx;
			y[t]=(int)yy;
		 }
